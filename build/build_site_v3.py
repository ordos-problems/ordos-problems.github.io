...
(function(){
  ...
  var q=document.getElementById('q'),cnt=document.getElementById('count'),selectedBox=document.getElementById('selectedtags'),clear=document.getElementById('clearfilter');
  function apply(){
    var t=q.value.trim().toLowerCase().replace(/#/g,'').split(/\s+/).filter(Boolean),shown=0;
    ...
    cnt.textContent='Showing '+shown+' of '+items.length+' problems'+(selectedTags.length?' tagged '+selectedTags.map(function(t){return '#'+t;}).join(' or '):'');
  }
  q.addEventListener('input',apply);
  clear.addEventListener('click',function(){selectedTags=[];q.value='';tagselect.value='';apply();});
  apply();
})();
</script>
</body>
